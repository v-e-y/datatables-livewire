<div wire:click.prefetch="toggle('{{ $index }}')"
     ...
     <button class="position-relative h-12 w-3">
         <span
             class="w-32 hidden group-hover:inline-block position-absolute z-10 top-0 left-0 ml-3 bg-blue-300 font-medium text-xs text-start text-blue-700  transform text-uppercase">
...
<div class="@if($column['hidden']) hidden @else position-relative h-12 overflow-hidden align-top d-flex d-table-cell @endif" @include('datatables::style-width')>
    ...
    @if($column['sortable'])
        <button wire:click="sort('{{ $index }}')"
                class="w-100 h-full px-6 py-3 border-b border-gray-200 bg-gray-50 text-xs font-medium text-gray-500 text-uppercase  d-flex justify-content-between items-center">
            <span class="inline flex-grow @if($column['headerAlign'] === 'right') text-end @elseif($column['headerAlign'] === 'center') text-center @endif"">{{ str_replace('_', ' ', $column['label']) }}</span>
    ...
    @else
        <div class="w-100 h-full px-6 py-3 border-b border-gray-200 bg-gray-50 text-xs font-medium text-gray-500 text-uppercase  d-flex justify-content-between items-center">
            <span class="inline flex-grow @if($column['headerAlign'] === 'right') text-end @elseif($column['headerAlign'] === 'center') text-center @endif"">{{ str_replace('_', ' ', $column['label']) }}</span>
        </div>
    @endif
