<div class="w-100">
    ...
    >
        <div class="sm:flex flex-grow sm:space-x-4">
            <div class="sm:w-1/3">
                <label
                    class="block uppercase tracking-wide text-xs font-bold py-1 rounded d-flex justify-content-between">Column</label>
                <div class="position-relative">
                    <select wire:model="rules.{{ $key }}.column" name="selectedColumn"
                        class="w-100 my-1 text-sm text-gray-900 block rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        <option value=""></option>
            ...
            @if ($options = $this->getOperands($key))
                <div class="sm:w-1/3">
                    <label
                        class="block uppercase tracking-wide text-xs font-bold py-1 rounded d-flex justify-content-between">Operand</label>
                    <div class="position-relative">
            ...
            @if (!in_array($rule['content']['operand'], ['is empty', 'is not empty']))
                <div class="sm:w-1/3">
                    @if ($column = $this->getRuleColumn($key))
                        <label
                            class="block uppercase tracking-wide text-xs font-bold py-1 rounded d-flex justify-content-between">Value</label>
                        <div class="position-relative">
                            ...
                            @elseif($column['type'] === 'date')
                                <input type="date" name="value" wire:model.lazy="rules.{{ $key }}.value"
                                    class="w-100 px-3 py-2 border my-1 text-sm text-gray-900 block rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" />
